...
## Set up a project before you build in it
...
Onboarding is the same list, but look at what's there first. Match the tools the repo already uses instead of swapping them out mid-project, and only add the missing pieces. If it already uses npm with a committed lockfile, don't switch it to pnpm without asking.
