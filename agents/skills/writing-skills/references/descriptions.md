...
Should-not-trigger queries are most valuable as near-misses: prompts that share keywords but need something different. For a CSV analysis skill, "update the formulas in my Excel budget spreadsheet" needs Excel editing, and "write a python script that reads a csv and uploads each row to postgres" is database ETL, not analysis. Obvious misses like "what's the weather" test nothing.
