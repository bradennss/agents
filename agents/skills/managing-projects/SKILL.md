...
Use [beads](https://github.com/steveyegge/beads) (the `bd` command) to track tasks instead of a markdown checklist. It keeps a dependency-aware issue graph in git, so plans survive across sessions, compaction, and crashes, and several agents can work without stepping on each other.
...
That prints bd's up-to-date workflow context (roughly 80 lines): the create, claim, and close loop, dependency commands, and current conventions. Follow what it says. If `bd prime` prints nothing or errors, bd isn't set up in this repo yet, so see below.
...
Run `bd setup --list` to see every supported agent. After setup, run `bd prime` and carry on.
...
Always run `bd prime` first, since these commands and their flags can change with the installed version.
